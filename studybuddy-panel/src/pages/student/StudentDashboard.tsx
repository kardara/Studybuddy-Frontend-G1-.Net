...
export default function StudentDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      try {
        quizAttemptsData = await quizzesService.getStudentQuizAttempts();
      ...
      try {
        // Replace with actual service when available
      ...
      // Calculate quiz statistics from quiz attempts
      const completedQuizzes = quizAttemptsData.filter(q => q.isPassed);
      const averageQuizScore = completedQuizzes.length > 0
        ? Math.round(
            completedQuizzes.reduce((sum, q) => sum + q.percentageScore, 0) / completedQuizzes.length
          )
        : 0;
        ...
        {detailedProgress.length > 0 ? (
          <div className="p-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl border border-primary/20">
            ...
            <h3 className="text-xl font-semibold mb-2">
              {detailedProgress[0].courseTitle}
            </h3>
            <p className="text-muted-foreground mb-4">
              {detailedProgress[0].completedLessons} of {detailedProgress[0].totalLessons} lessons
              completed
            </p>
            <ProgressBar value={detailedProgress[0].progressPercentage} className="mb-4" showLabel={false} />
            <div className="flex items-center justify-between">
              ...
              <Link to={`/student/courses/${detailedProgress[0].courseId}`} className="btn-primary">
